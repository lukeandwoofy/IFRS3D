...
async function initSim() {
  ...
  // Load aircraft model
  const airplaneUri = await Cesium.IonResource.fromAssetId(3701524); // Replace with your asset ID
  ...
  // Only fly to the model after it's ready
  if (planeEntity.model) {
    ...
    console.error("Model failed to load — check your asset ID and token.");
  }
  ...
  viewer.clock.onTick.addEventListener(() => {
    ...
    if (viewMode === 'third') {
      ...
      viewer.camera.lookAtTransform(transform, new Ces
